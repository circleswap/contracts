...
contract UniswapV2ERC20 is IUniswapV2ERC20, Initializable {
    ...
    string public constant name = 'CircleSwap Liquidity Provider Token';
    string public constant symbol = 'CSLPT';
...
contract CircleSwapPair is IUniswapV2Pair, UniswapV2ERC20 {
    using SafeMath  for uint;
    ...
    uint private locked;
    modifier lock() {
        require(locked == 0, 'CircleSwap: LOCKED');
        locked = 1;
        _;
        locked = 0;
    }
    ...
    // if fee is on, mint liquidity equivalent to 3/7th of the growth in sqrt(k)
...
contract CircleSwapFactory is IProxyFactory, IUniswapV2Factory {
    ...
    function initialize(address _feeToSetter, address _productImplementation) public {
        require(feeToSetter== address(0) && _feeToSetter != address(0), 'CircleSwapFactory.initialize can be delegatecall once by proxy only.');
...
contract CircleSwapRouter02 is IUniswapV2Router01, IUniswapV2Router02, Initializable {
    using SafeMath for uint;
...
contract DeployFactory {
    ...
    constructor(address adminFactory, address adminPair) public {
        CircleSwapPair pair = new CircleSwapPair();
        emit Deploy('CircleSwapPair', address(pair));
        
        CircleSwapFactory factory = new CircleSwapFactory();
        emit Deploy('CircleSwapFactory', address(factory));
